	...
	if(rdr->lb_stat)
	{
		...
		for(; rowcount < statsize; ++rowcount)
		{
			...
			if(!(s->rc == rc2hide))
			{
				...
				localtime_r(&walltime, &lt); // fixme we need walltime!
				...
				else
				{
					...
					int64_t gone = comp_timeb(&s->last_received, &lastaccess);
					if(gone > 0)
				...
				if(!apicall)
				{
					if(s->rc == E_NOTFOUND)
					{
						...
						tpl_printf(vars, TPLAPPEND, "READERSTATSNFHEADLINE", "<TD CLASS=\"subheadline\" colspan=\"3\"><A HREF=\"readerstats.html?label=%s&amp;action=resetstat&amp;rc=4\">delete all %s</A></TD></TR>\n",
					...
					else if(s->rc == E_TIMEOUT)
					{
						...
						tpl_printf(vars, TPLAPPEND, "READERSTATSTOHEADLINE", "<TD CLASS=\"subheadline\" colspan=\"3\"><A HREF=\"readerstats.html?label=%s&amp;action=resetstat&amp;rc=5\">delete all %s</A></TD></TR>\n",
								   urlencode(vars, rdr->label),
								   stxt[s->rc]);
					}
					else
...
static char *send_oscam_user_config(struct templatevars *vars, struct uriparams *params, int32_t apicall)
{
	...
	for(account = cfg.account; (account); account = account->next)
	{
		...
		for(cl = first_client->next; cl ; cl = cl->next)
		{
			...
			{
				int64_t gone_lastecm = comp_timeb(&cl->lastecm, &latestactivity);
				int64_t gone_login = comp_timeb(&cl->login, &latestactivity);
				if(gone_lastecm > 0 || gone_login > 0)
				{
					int64_t gone_ecm_login = comp_timeb(&cl->lastecm,&cl->login);
...
static char *send_oscam_status(struct templatevars * vars, struct uriparams * params, int32_t apicall)
{
	...
	char picon_name[32];
	tpl_printf(vars, TPLADD, "SVNREV", CS_SVN_VERSION);
	...
	for(i = 0, cl = first_client; cl ; cl = cl->next, i++)
	{
		...
		if(get_module(cl)->listenertype != LIS_CSPUDP)
		{
			...
			if(cl->wihidden != 1)
			{
				int64_t gone_lastecm = comp_timeb(&now,&cl->lastecm);
				...
				if(!filtered)
				{
					...
					if((isec < cfg.hideclient_to || cfg.hideclient_to == 0) && (cl->typ == 'c' || cl->typ == 'p' || cl->typ == 'r'))
					{
						...
						{
							tpl_printf(vars, TPLADD, "CLIENTLBVALUE", "(%jdms)<br>", cl->cwlastresptime);
...
static char *send_oscam_failban(struct templatevars * vars, struct uriparams * params, int8_t apicall)
{
	...
	while((v_ban_entry = ll_iter_next(&itr)))
	{
		...
		localtime_r(&walltime, &st); // fix me, we need walltime!
		if(!apicall)
		{
...
static char *send_oscam_image(struct templatevars * vars, FILE * f, struct uriparams * params, char *image, time_t modifiedheader, uint32_t etagheader, char *extraheader)
{
	...
	if(strlen(wanted) > 3 && wanted[0] == 'I' && wanted[1] == 'C')
	{
		if(etagheader == 0)
		{
			...
			if(disktpl == 0 && walltime < modifiedheader) // fixme we need walltime!
			{
				send_header304(f, extraheader);
				return "1";
			}
		...
		if(written)
		{
			...
			if(cl->ll_cacheex_stats)
			{
				...
				while((cacheex_stats_entry = ll_iter_next(&itr)))
				{
					...
					int64_t gone = comp_timeb(&now, &cacheex_stats_entry->cache_last);
...
static int32_t process_request(FILE * f, IN_ADDR_T in)
{
	...
	do
	{
		...
		else
		{
			...
			time_t walltime = cs_walltime(&first_client->login);
			localtime_r(&walltime, &st); // fixme we need walltime
...
static void *http_server(void *UNUSED(d))
{
	...
	cs_lock_create(&http_lock, 10000, "http_lock");
	init_noncelocks();
